...
from api.models import Category, Subcategory, Gig, UserProfile, Job, Conversation, Message, Proposal
...
for category_name, subcategory_list in subcategories_data.items():
    try:
        ...
        for subcategory_name in subcategory_list:
            subcategory, created = Subcategory.objects.get_or_create(
                ...
                defaults={'description': f'Specialized services in {subcategory_name.lower()}'}
            )
            if created:
                print(f"Created subcategory: {subcategory_name[:50]}...")
...
print(f"Created {Subcategory.objects.count()} subcategories")
print("\nYou can now:")
